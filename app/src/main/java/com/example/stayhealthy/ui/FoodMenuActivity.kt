...
class FoodMenuActivity : AppCompatActivity(), FoodMenuFragment.OnFoodAdd, ChangeFoodQuantityDialog.ChangeQuantityDialogEvents {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent != null) {
            date = intent.extras?.getLong(MEAL_DATE, DATE_DEFAULT)
            userId = intent.extras?.getString(CURRENT_USER_ID)
        }
    ...
    override fun onFoodAdd(foodItem: Food, category: String) {
        ...
        val args = Bundle().apply{
            putInt(CHANGE_QUANTITY_ID, DIALOG_CHANGE_QUANTITY)
            putParcelable(CHANGE_QUANTITY_FOOD, foodItem)
            putString(CHANGE_QUANTITY_FOOD_CATEGORY, category)
        }
        dialog.arguments = args
        dialog.show(supportFragmentManager, "changeQuantityDialog")
    ...
    override fun onAddDialogResult(dialogId: Int, args: Bundle) {
        ...
        if(dialogId == DIALOG_CHANGE_QUANTITY) {
            val food = args.getParcelable(CHANGE_QUANTITY_FOOD) as Food?
            val category = args.getString(CHANGE_QUANTITY_FOOD_CATEGORY)
