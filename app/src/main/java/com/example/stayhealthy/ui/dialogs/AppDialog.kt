...
class AppDialog : AppCompatDialogFragment() {
    ...
    internal interface DialogEvents {
        fun onPositiveDialogResult(dialogId: Int, args: Bundle) // id is used so you can identify which dialog will be used in activity, activity will provide ids and it can then recognize different dialogs
...
//        fun onDialogCancelled(dialogId: Int)
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if (arguments != null) {
            ...
            messageString = arguments.getString(DIALOG_MESSAGE) //using string not string resource

            if (dialogId == 0 || messageString == null) { // if either of arguments are missing it return 0 and null
                throw IllegalArgumentException("DIALOG_ID and/or DIALOG_MESSAGE not present in the bundle")
            }

            positiveStringId = arguments.getInt(DIALOG_POSITIVE_RID) // default values if text for button isn't provided
    ...
    override fun onCancel(dialog: DialogInterface) { // when it is back button clicked this method is called
        Log.d(TAG, "onCancel called")
        val dialogId = requireArguments().getInt(DIALOG_ID) // you don't need to check if arguments exist it was already checked when creating dialog
